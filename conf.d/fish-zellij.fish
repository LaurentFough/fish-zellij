...
#= file: $HOME/.config/fish/conf.d/fish-zellij.fish


#= `__zellij::` is used as the namespace prefix for all functions in this file to avoid
#= name collisions and to be "hidden" from the user. That is, it seems very improbable
#= that a user would have any other function or program on their system with the same
#= prefix, leading to some annoyance when tab completing function names in interactive mode.
...
set --global ZELLIJ_FISH_TITLE "fish-zellij.fish"                                                                #= Used for notifications and the border label in fzf
